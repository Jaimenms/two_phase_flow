...
class TestStratifiedTwoPhaseFlow(TestCase):
    ...
    def test_1(self, plot=True):
        ...
        t, y, yp = DasslSolver.run(m, t, y0, display=True, rtol=1e-6, atol=1e-3, user_jacobian=True)
    ...
    def test_2(self, plot=True):
        ...
        m.parameters['rhoL'].set(1000, "kg/m**3")
        m.parameters['muL'].set(0.001, "Pa*s")
        m.parameters['drhoLdP'].set(5e-7*Pr, "kg/m**3/Pa")
        m.parameters['rhoG'].set(1, "kg/m**3")
        m.parameters['muG'].set(0.00001, "Pa*s")
        m.parameters['drhoGdP'].set(5e-4*Pr, "kg/m**3/Pa")
        m.parameters['qL_lb'].set(value_span=qLi * np.array([1, 1, 1.01, 1.01]), value_unit="kg/s", time_unit="s", time_span=np.array([0, 2, 3, 10000]))
        m.parameters['qG_lb'].set(value_span=qGi * np.array([1, 1, 1.01, 1.01]), value_unit="kg/s", time_unit="s", time_span=np.array([0, 2, 3, 10000]))
        m.parameters['P_ub'].set(value_span=Pf * np.array([1, 1, 1, 1]), value_unit="Pa", time_unit="s", time_span=np.array([0, 2, 3, 10000]))

        tetha = np.ones_like(x)
        tetha[x<50] = 2*np.pi*(-5/360)
        tetha[x>=50] = 2*np.pi*(5/360)
        ...
        m.variables["alphaL"].set_ic(value=alphaL0, unit="")
        P0 = np.linspace(Pi, Pf, N)
        m.variables["P"].set_ic(value=P0/Pr, unit="Pa")

        y0 = m.variables.get_ic_array()

        t0, y0, yp0 = DasslSolver.run(m, None, y0, display=True, rtol=1e-3, atol=1e-6)

        t, y, yp = DasslSolver.run(m, t, y0, display=True, rtol=1e-2, atol=1e-2, user_jacobian=True)
